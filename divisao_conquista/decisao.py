...
def bin_packing(items, bin_capacity, k):
    # Caso base: se há apenas um item ou nenhum, retorna se é possível acomodar no número de contêineres permitido
    if len(items) <= 1:
        return len(items) <= k
    ...
    # Retorna se o número total de compartimentos necessários é menor ou igual a k
